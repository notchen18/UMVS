...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($roomStmt = $conn->prepare($roomQuery)) {
        $roomStmt->bind_param("s", $RoomName);
        $roomStmt->execute();
    ...
    $subjectQuery = "SELECT SubjectID FROM subjects WHERE SubjectCode = ?";
    ...
    if ($sectionStmt = $conn->prepare($sectionQuery)) {
        $sectionStmt->bind_param("s", $SectionCode);
        $sectionStmt->execute();
